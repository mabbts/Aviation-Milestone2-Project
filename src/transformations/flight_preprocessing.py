...
def compute_track_metrics(track):
    ...
    metrics = {
        ...
        "time_max":    times.max(),
        "time_avg":    times.mean(),
        # Convert to datetime and floor the precision to milliseconds.
        "start_time":  pd.to_datetime(times.min(), unit="s").floor('ms'),
        "end_time":    pd.to_datetime(times.max(), unit="s").floor('ms'),
        # Latitude metrics
